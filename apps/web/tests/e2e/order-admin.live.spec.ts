...
test.describe("order admin live flow", () => {
  ...
  test("keeps detail columns stacked below 520px", async ({ page }) => {
    await page.setViewportSize({ width: 500, height: 900 });
    await openFirstOrder(page);

    await expect(
      page.getByRole("heading", { name: "Согласия", exact: true }),
    ).toHaveCount(0);

    const mainColumn = page.getByTestId("order-main-column");
    const sideColumn = page.getByTestId("order-side-column");

    await expect
      .poll(async () => {
        const [mainBox, sideBox] = await Promise.all([
          mainColumn.boundingBox(),
          sideColumn.boundingBox(),
        ]);
        if (!mainBox || !sideBox) return null;
        return {
          sameLeftEdge: Math.abs(mainBox.x - sideBox.x) <= 1,
          sameWidth: Math.abs(mainBox.width - sideBox.width) <= 1,
          stacked: sideBox.y > mainBox.y,
        };
      })
  ...
  test("manager edits an order and restores its original values", async ({
    ...
    for (let cycle = 1; cycle <= editCycles; cycle += 1) {
      ...
      const total = dialog.getByText(/^Итого:/);
      const originalTotalText = await total.textContent();
      expect(originalTotalText).not.toBeNull();
      ...
      try {
        ...
        await expect(total).not.toHaveText(originalTotalText!);
