---
name: brega-deploy
description: Deploy the Brega Chai project through either the normal GitHub pull-request and release workflow or an explicitly selected direct production rollout. Use whenever the user asks to deploy, redeploy, release, or ship brega-chai; combines commit or push with a production rollout; or mentions GitHub Actions, GHCR, release workflows, registry images, production SSH, or deploy checks. Do not use for a standalone local commit or non-production push.
---
...
1. **Normal via PR and workflows (recommended)** — push a feature branch, create a PR, let GitHub Actions run its checks, and deploy production only from an approved `release-X.Y.Z` tag.
2. **Direct manual rollout** — after explicit approval, build immutable images locally, push them to GHCR, and deploy over SSH with local and production checks replacing the release workflow. Do not weaken branch or tag protections to use this path.

If the choice is not explicit, ask this question immediately and do not perform mutations until the user answers:

> Как деплоим: обычно через PR и GitHub Actions или прямым ручным rollout?

Use `request_user_input` when available. Put the normal path first and mark it recommended. If the user already said “через PR/Actions” or “напрямую/мимо workflow”, use that choice without asking again.
...
3. Determine affected services from the actual diff:
   ...
   - Compose, `infra`, `scripts`, or `.github/workflows/release.yml` are deployment changes, not ordinary application-only changes.
...
## Normal PR and workflow path

1. Do not use skip directives.
2. Work on a feature branch. Do not place a new PR change directly on `main`.
3. Commit and push the branch normally.
4. Create a GitHub PR with a concise title, structured Russian description, verification evidence, and a conventional squash-commit message.
5. Monitor the PR checks and report failures with relevant job evidence. Do not merge unless the user explicitly authorizes merging.
...
7. The release workflow must build both immutable images, smoke-test them, run `scripts/deploy.sh`, and pass the production job. Do not substitute a manual SSH rollout inside this path.
8. Perform the production verification described below after the release workflow succeeds.

## Direct manual rollout path

Use this path only after explicit selection. It intentionally replaces the GitHub release workflow with local evidence.

### Commit and push

1. Confirm the target commit is on `main`, fetch the GitHub remote, and check for divergence before pushing. Never force-push and never bypass repository rulesets.
2. Commit only verified task changes.
3. Push normally if a push is still required; do not encode CI bypasses in commit messages.
4. Record the full commit SHA and state explicitly that the release workflow was bypassed.
...
```text
ghcr.io/aleksandr190101996/brega-chai-web:<full-sha>
ghcr.io/aleksandr190101996/brega-chai-cms:<full-sha>
```
...
docker buildx build --platform linux/amd64 --target production \
  -f apps/web/Dockerfile -t ghcr.io/aleksandr190101996/brega-chai-web:<full-sha> \
  --push .
docker buildx build --platform linux/amd64 --target production \
  -f apps/cms/Dockerfile -t ghcr.io/aleksandr190101996/brega-chai-cms:<full-sha> \
  --push .
```
...
5. For the direct path, report that the release workflow was bypassed; for the normal path, confirm the release workflow and deploy job succeeded.
