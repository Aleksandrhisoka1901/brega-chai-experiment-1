...
После подтверждённого результата заказа BFF синхронно инвалидирует tag
`products`, возможные `product-slug:{type}:{slug}` и связанные storefront paths
до отправки успешного ответа. Клиент затем фоново запрашивает live-остатки и
...
заказ и не попадает в публичный ответ.
...
- live-проверка остатка при переходе к checkout;
- обновление карточки после заказа без page navigation;
- double submit;
- keyboard-only;
- mobile swipe/drawer;
